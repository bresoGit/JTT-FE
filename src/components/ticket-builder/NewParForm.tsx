...
export interface MarketOption {
  code: string; // već je labela s backend-a
  odds: number;
}

interface NewParFormProps {
  isAdding: boolean; // više ne koristimo za prikaz, ali ostavljamo potpis radi kompatibilnosti
...
const NewParForm: React.FC<NewParFormProps> = ({
  // isAdding – ignoriramo za prikaz, forma je uvijek vidljiva
  disabled,
  newPar,
  countries,
  leagues,
  ...
  // label = m.code BEZ dodatnog formatiranja
  ...
  return (
    <section className="rounded-2xl border border-jack-border bg-black/60 p-4">
      ...
      <p className="mb-3 text-[11px] text-slate-400">
        Odaberi sport, državu, ligu, dan, utakmicu i tip oklade. Dok Jack
        skenira listić, dodavanje novih parova je privremeno zaključano.
      </p>

      {/* Forma je SADA UVIJEK vidljiva */}
      <form onSubmit={onSubmit} className="space-y-3 text-xs md:text-sm">
        <fieldset
          disabled={effectiveDisabled}
          className={effectiveDisabled ? "opacity-50 pointer-events-none" : ""}
        >
          {/* Sport */}
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            >
              {/* ako želiš, promijeni tekst ovdje */}
              Dodaj par na listić
            </button>
          </div>
        </fieldset>
