 ...
 * - match (two team logos + league + time)
 */
  ...
  return (
    <div className="flex flex-col gap-1" ref={containerRef}>
      ...
      <button
        ...
        disabled={isDisabled}
        onClick={toggleOpen}
        className={`flex w-full items-center justify-between gap-2 rounded-xl border px-3 py-2 text-left text-xs text-slate-100
          ${isDisabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}
          ${
            error
              ? "border-red-500 bg-black/80"
              : "border-jack-border bg-black/80 hover:border-red-400"
          }`}
      >
      ...
      {open && (
        <div className="mt-1 max-h-72 w-full overflow-hidden rounded-xl border border-jack-border bg-black/95 text-xs shadow-[0_0_22px_rgba(0,0,0,0.9)]">
          {/* 🔎 search input */}
          {searchable && (
